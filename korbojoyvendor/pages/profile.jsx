...
function Profile(props) {
  ...
  const onSubmit = async (e) => {
    ...
    const info = { jsdlk: userInfo["0"].id, user: user };

    const response = await axios
      .post(process.env.API_URL + "/vendorPanel/v1/profileInformationUpdate", info)
      .then((response) => {
                      ...
                      <form onSubmit={onSubmit} method="post" className="form-horizontal" encType="multipart/form-data">
                        <div className="form-row">
                          ...
                          <div className="form-group col-md-12">
                            ...
                            <input type="text" className="form-control" id="add2" defaultValue={userInfo["0"].district} name="districts" />
                          </div>
...
export async function getServerSideProps({ req }) {
  ...
  const { data } = await axios.get(process.env.API_URL + "/vendorPanel/v1/agentInformation/" + decodedToken.userId);
  if (!data) {
    return {
      notFound: true,
    };
